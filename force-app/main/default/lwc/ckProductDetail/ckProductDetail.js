...
export default class CkProductDetail extends LightningElement {
  ...
  get favoriteClass() {
    return this.isFavorited
      ? "detail-fav-btn detail-fav-active"
      : "detail-fav-btn";
  }
  get hasRelated() {
  ...
  get stockClass() {
    if (!this.stockInfo) return "";
    if (this.stockInfo.availability === "En stock")
      return "stock-badge stock-instock";
    if (this.stockInfo.availability === "Epuise")
      return "stock-badge stock-out";
    if (this.stockInfo.availability === "En arrivage")
      return "stock-badge stock-arriving";
    return "stock-badge stock-order";
  }

  get stockQuantityText() {
    if (!this.stockInfo || this.stockInfo.availability !== "En stock")
      return "";
    if (this.stockInfo.stockQuantity <= 5)
      return this.stockInfo.stockQuantity + " left - order soon!";
