...
const STANDARD_LOGIN_URL = "/TalFlowExpress/s/login";
...
export default class CkAuthModal extends LightningElement {
  ...
  get userPhone() {
    return this.userData ? this.userData.phone : "";
  }
  get userPhoto() {
    return this.userData ? this.userData.photoUrl : "";
  }
  get userProfileName() {
    return this.userData ? this.userData.profileName : "";
  }
  get userCompany() {
    return this.userData ? this.userData.companyName : "";
  }
  get userMemberSince() {
    return this.userData ? this.userData.memberSince : "";
  }
  get userContactId() {
    return this.userData ? this.userData.contactId : "";
  }
  get showUserPhoto() {
    return this.isLoggedIn && this.userData && this.userData.photoUrl;
  }

  // ═══ Getters - Steps ═══
  get isProfileStep() {
    return this.currentStep === "profile";
  }
  get isChoiceStep() {
    return this.currentStep === "choice";
  }
  get isAccountTypeStep() {
    return this.currentStep === "accounttype";
  }
  get isRegisterB2CStep() {
    return this.currentStep === "registerb2c";
  }
  get isRegisterB2BStep() {
    return this.currentStep === "registerb2b";
  }
  get isSuccessStep() {
    return this.currentStep === "success";
  }
  get isConfirmLogoutStep() {
    return this.currentStep === "confirmlogout";
  }
  get isLoggingOutStep() {
    return this.currentStep === "loggingout";
  }
  get isVerifyStep() {
  ...
  // ═══ Navigation ═══
  // Sign In → toujours redirige vers la page login standard de Salesforce
  showSignIn() {
    this.closeModal();
    window.location.href = STANDARD_LOGIN_URL;
  }
  ...
  showAccountType() {
    this.currentStep = "accounttype";
    this.errorMessage = "";
  }
  selectB2C() {
    this.currentStep = "registerb2c";
    this.errorMessage = "";
  }
  selectB2B() {
    this.closeModal();
  ...
  // "Back to Sign In" depuis l'\u00e9tape verify → redirige aussi vers standard
  handleBackToSignIn() {
    this.closeModal();
    window.location.href = STANDARD_LOGIN_URL;
  }
  ...
  handleVerifyCode() {
    ...
    verifyEmail({ email: this.verificationEmail, code: this.verificationCode })
      ...
      .catch((error) => {
        this.isVerifying = false;
        this.verificationError = error.body
          ? error.body.message
          : "Verification failed.";
      });
  }
  ...
  handleResendCode() {
    ...
    resendVerificationCode({ email: this.verificationEmail })
      .then(() => {
        this.verificationMessage =
          "A new code has been sent to " + this.verificationEmail;
      })
      .catch((error) => {
        this.verificationError = error.body
          ? error.body.message
  ...
  // ═══ Logout ═══
  confirmLogout() {
    this.currentStep = "confirmlogout";
  }
  cancelLogout() {
    this.currentStep = "profile";
  }
  ...
  loginWithGoogle() {
    window.location.href =
      "/TalFlowExpress/services/auth/sso/Google?startURL=/TalFlowExpress/s/";
  }

  //   loginWithFacebook() {
  //     window.location.href =
  //       "/TalFlowExpress/services/auth/sso/Facebook?startURL=/TalFlowExpress/s/";
  //   }
